...
const Home = () => {
    ...
    const titles = [
        ...
        <h3 className="text-3xl 2xs:text-4xl xs:text-5xl md:text-6xl lg:text-7xl 2xl:text-8xl font-bold opacity-50 dark:opacity-70 py-2">A full-stack developer.</h3>,
        ...
        <div className="text-left mt-[10vh]">
            <TransitionGroup>
                {isMounted && titles.map((title, i) => <CSSTransition key={i} classNames="fadeup" timeout={2000}>
                    <div key={i} style={isAppearing ? { transitionDelay: `${i * 100}ms` } : {}}>{title}</div>
                </CSSTransition>)}
            </TransitionGroup>
        </div>
    </section>;
